...
namespace methhead
{
    ...
    void ActorBase::pickup(const SimContext &, const IPickup & pickup)
    {
        m_score += pickup.scoreChange();
        m_moveDelaySec *= pickup.speedChangeMult();
    }
    ...
    bool ActorBase::move(const SimContext & context)
    {
        ...
        // clang-format off
        std::array<WalkDistance, 4> moves = {
            WalkDistance{ (currPos + sf::Vector2i(-1,  0)), 0 },
            WalkDistance{ (currPos + sf::Vector2i( 1,  0)), 0 },
            WalkDistance{ (currPos + sf::Vector2i( 0, -1)), 0 },
            WalkDistance{ (currPos + sf::Vector2i( 0,  1)), 0 } };
        // clang-format on

        // remove any off-board moves
        WalkDistVec_t::iterator endIter =
            std::remove_if(std::begin(moves), std::end(moves), [&](const WalkDistance & walkDist) {
        ...
        endIter = std::remove_if(std::begin(moves), endIter, [&](const WalkDistance & walkDist) {
        ...
        if (std::begin(moves) == endIter)
        {
            return false;
        }

        // finish here if there is only one possible move
        if ((std::begin(moves) + 1) == endIter)
        {
            ScopedBoardPosition::set(context, moves[0].pos);
            return true;
        }

        assert(std::distance(std::begin(moves), endIter) > 1);

        // calc how far each move is from the target
        for (auto iter(std::begin(moves)); iter != endIter; ++iter)
        {
        ...
        endIter = std::remove_if(std::begin(moves), endIter, [&](const WalkDistance & posDist) {
            return posDist.dist > shortestWalkDist;
        });

        // random select one
        const BoardPos_t newPos{ context.random.from(std::begin(moves), endIter).pos };
